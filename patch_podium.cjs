...
const updatedRank1 = `                             {/* Rank 1 */}
                             <div className="w-[45%] flex flex-col items-center justify-end relative z-30 mx-[-8px]">
                                <div className="absolute top-[-34px] z-40 text-4xl overflow-visible flex justify-center items-center h-12 w-12 drop-shadow-[0_2px_4px_rgba(0,0,0,0.6)]">
...
const crownIconFix = `                                {/* Background graphics for the wingo icon imitation */}
...
content = content.replace(
  /<div className="w-1\/3 flex flex-col items-center justify-end relative z-10">[\s\S]*?<img src=\{MOCK_PODIUM\[2\].img\}[\s\S]*?<\/div>\s*<\/div>/,
  updatedRank3
);

// Replace Rank 1
let replaced1 = content.replace(
  /<div className="w-\[45%\] flex flex-col items-center justify-end relative z-30 mx-\[-8px\]">[\s\S]*?<img src=\{MOCK_PODIUM\[0\].img\}[\s\S]*?<\/div>\s*<\/div>/,
  updatedRank1
);
if (replaced1 === content) {
    console.log("Failed to substitute rank 1 properly, maybe emoji wasn't escaped right. Looking for alternative match.");
    // fallback
    replaced1 = content.replace(/<div className="w-\[45%\] flex flex-col items-center justify-end relative z-30 mx-\[-8px\]">[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/, updatedRank1 + "\n\n                             {/* Rank 2 will be replaced next line maybe */}");
}
content = replaced1;
...
// Replace Rank 2
content = content.replace(
  /<div className="w-1\/3 flex flex-col items-center justify-end relative z-10">[\s\S]*?<img src=\{MOCK_PODIUM\[1\].img\}[\s\S]*?<\/div>\s*<\/div>/g,
  updatedRank2
);

// We should run regex directly for Rank 1 instead of assuming it matches easily
content = content.replace(/<div className="absolute top-\[-30px\] z-40 text-4xl">👑<\/div>/, `<div className="absolute top-[-34px] z-40 text-4xl overflow-visible flex justify-center items-center h-12 w-12 drop-shadow-[0_2px_4px_rgba(0,0,0,0.6)]">
                                  <svg xmlns="http://www.w3.org/2000/svg" width="44" height="44" viewBox="0 0 24 24">
                                    <defs>
                                      <linearGradient id="gold" x1="0%" y1="0%" x2="0%" y2="100%">
                                        <stop offset="0%" stopColor="#FFF200" />
...
// Fix crown emoji in winners list
content = content.replace(
  /<div className="absolute inset-0 opacity-40 blur-\[1px\] z-10 flex items-center justify-center text-\[30px\]">\s*👑\s*<\/div>/g,
  crownIconFix
);
